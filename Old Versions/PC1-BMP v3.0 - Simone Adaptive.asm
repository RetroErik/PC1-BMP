; ============================================================================
; PC1-BMP4.ASM (v 3.0) - BMP Viewer with Simone Technique + Adaptive Streaming
; ============================================================================
;
...
;   This maximizes stream truncation: if C doesn't change between lines,
;   we skip writing entries 6/7. If B also unchanged, skip 4/5 too.
;   If nothing changes: ZERO writes = ZERO blinking on that line!
;
...
;   4b. BUILD STREAM LENGTHS: Compare consecutive stream entries,
;       determine per-line write count (0 or 12 bytes)
...
; ============================================================================
; Compares consecutive palette_stream entries (12 bytes each).
; If ALL 12 bytes match → stream_len = 0 (skip: no palette write needed)
; Otherwise → stream_len = 12 (full write: all entries E2-E7)
;
; Line 0 always gets stream_len = 12 (must write initial state).
;
; When stream_len = 0, the V6355D registers already contain the correct
; values from the last line that wrote them. Since we only skip when
; the stream data is IDENTICAL, the registers need no update.
;
...
build_stream_lengths:
    ...
    ; Compare lines 1-199 to their predecessor
    mov si, palette_stream          ; SI = previous line (L-1)
    mov di, palette_stream + 12     ; DI = current line (L)
    mov bx, stream_len + 1          ; BX = output pointer
    mov cx, 199

.bsl_loop:
    ; Compare all 12 bytes (6 word comparisons)
    mov ax, [si]
    cmp ax, [di]
    jne .bsl_full
    mov ax, [si + 2]
...
; ============================================================================
; render_frame - Adaptive per-scanline HBLANK streaming
; ============================================================================
; SIMONE TECHNIQUE with SKIP OPTIMIZATION (V40/80186+):
;
;   Same proven OUTSB streaming as PC1-BMP3, but with one addition:
;   lines where NO palette entries change are SKIPPED entirely.
;
;   For each scanline, checks stream_len[] (0 or 12):
;     stream_len = 12: Full write (identical to PC1-BMP3)
;     stream_len = 0:  SKIP — no palette writes, just flip
;
;   Skip lines have ZERO palette write noise — no protocol open,
;   no data writes, no close. Just the flip OUT instruction.
;
;   Colors are pre-sorted by stability (reorder_by_stability) so
;   consecutive lines with the same dominant colors get stream_len=0.
;   For images with stable regions (brick walls, sky, text boxes),
;   this can eliminate blinking on 30-50% of scanlines.
;
;   Branch overhead: only ~8 cycles (cmp + je) per line before the
;   palette write — negligible compared to the ~89-cycle HBLANK.
;
...
.rf_wait_high:
    ...
    ; === HBLANK: check if this line needs palette writes ===
    cmp byte [bp], 0
    je .rf_skip
    inc bp
...
.rf_skip:
    ; === No palette writes needed — zero noise! ===
    inc bp
...
msg_info    db 'PC1-BMP4 v3.0 - Simone + Adaptive Streaming', 0x0D, 0x0A
            db 0x0D, 0x0A
